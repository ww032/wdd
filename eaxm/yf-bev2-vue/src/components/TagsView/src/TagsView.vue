...
const moveToTarget = (currentTag: RouteLocationNormalizedLoaded) => {
  ...
  if ((firstTag?.to as RouteLocationNormalizedLoaded).fullPath === currentTag.fullPath) {
    // 直接滚动�?的位�?    const {start} = useScrollTo({
      el: wrap$!,
      position: 'scrollLeft',
      to: 0,
      duration: 500
...
// 右键菜单装填改变的时�?const visibleChange = (visible: boolean, tagItem: RouteLocationNormalizedLoaded) => {
  if (visible) {
