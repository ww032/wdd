...
export default defineComponent({
  ...
  props: {
    ...
    // 展开�?    // expand: propTypes.bool.def(false),
    // 是否展示分页
    pagination: {
      type: Object as PropType<Pagination>,
      default: (): Pagination | undefined => undefined
    },
    // 仅对 type=selection 的列有效，类型为 Boolean，为 true 则会在数据更新之后保留之前选中的数据（需指定 row-key�?    reserveSelection: propTypes.bool.def(false),
    // 加载状�?    loading: propTypes.bool.def(false),
    // 是否叠加索引
    ...
    // row-class-name, 类型�?(row: Recordable, rowIndex: number) => string | string
    rowClassName: {
    ...
    tooltipOptions: {
      type: Object as PropType<Pick<ElTooltipProps,
          | 'effect'
          | 'enterable'
          | 'hideAfter'
          | 'offset'
